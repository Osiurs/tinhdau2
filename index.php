...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $capThem = isset($_POST['cap_them']) ? 1 : 0;
        ...
        // Bỏ cơ chế khoảng cách thủ công
        $khoangCachThuCong = null;
        ...
        // Thực hiện tính toán
        if ($capThem) {
            // Lệnh Cấp thêm: tạo kết quả tiêu hao thực tế
            $ketQua = [
                ...
                'chi_tiet' => [
                    'sch' => 0,
                    'skh' => 0,
                    'cong_thuc' => $lyDoCapThem . " x " . $soLuongCapThem . " lít"
                ]
            ];
        } else {
            ...
            if ($doiLenh) {
                ...
                if (is_array($ketQua)) {
                    ...
                    $routeSegments[] = $diemKetThuc . ' (đổi lệnh)';
                    if (!empty($structuredDiemMoi)) {
                ...
                try {
                    $ketQua = $tinhToan->tinhNhienLieu($tenTau, $diemBatDauGoc, $diemKetThucGoc, $khoiLuong);
                } catch (Exception $e) {
                    $link = '<a href="admin/quan_ly_tuyen_duong.php">Quản lý tuyến đường</a>';
                    throw new Exception('Chưa có tuyến trực tiếp giữa "' . $diemBatDauGoc . '" và "' . $diemKetThucGoc . '". Vui lòng vào ' . $link . ' để thêm tuyến.');
                }
            }
        }
        ...
        // Khối lượng luân chuyển = quãng đường có hàng (Sch) × khối lượng (D)
        $khoiLuongLuanChuyen = (!$capThem && $sch > 0 && $khoiLuong > 0) ? ($sch * $khoiLuong) : 0;
        ...
        $dataLuu = [
            ...
            'cay_xang_cap_them' => '', // Không cần cây xăng vì dầu được múc từ trong tàu
            'ghi_chu' => $ghiChu,
            'ngay_di' => $ngayDi,
            'ngay_den' => $ngayDen,
            'ngay_do_xong' => $ngayDoXong,
        ...
        if ($action === 'save') {
            ...
            error_log('DEBUG SAVE ACTION: Data to be saved: ' . print_r($dataLuu, true));

            // Nếu chỉ cấp thêm (không có tuyến) => ghi nhận như 1 chuyến mới độc lập
            // vẫn dùng cùng cấu trúc CSV, Sch/Skh/nhien_lieu có thể là 0
            file_put_contents('debug_log.txt', "[POST] Attempting to save data...\n", FILE_APPEND);
            $saved = $luuKetQua->luu($dataLuu);
            
            if (!$saved) {
                error_log('DEBUG SAVE ACTION: luu() returned false.');
            }

            if ($saved) {
                // Lệnh Cấp thêm không cần ghi vào sổ dầu tồn vì dầu được múc từ trong tàu
                // Điều hướng về trang chính với tàu và số chuyến để hiển thị ngay các đoạn của chuyến
                // Sử dụng $soChuyen hiện tại thay vì lấy mã cao nhất
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form method="POST" action="" onsubmit="return validateForm()" autocomplete="off">
                                                    ...
                                                    <?php
                                                        ...
                                                        elseif ($doiLenhTuyenJson !== '' && $isDoiLenh) {
                                                            ...
                                                            if (is_array($doiLenhTuyenData) && !empty($doiLenhTuyenData)) {
                                                                ...
                                                                if ($diemDuKien !== '') {
                                                                    $routeSegments[] = $diemDuKien . ' (đổi lệnh)';
                                                                }
                                    ...
                                    <div class="col-lg-4 col-md-5">
                                        <div class="d-flex flex-wrap gap-1 align-items-center reason-group">
                                            <input type="text" class="form-control form-control-sm diem-moi-reason" name="diem_moi_reason[]"
                                                placeholder="Lý do (ví dụ: Đổi lệnh)" autocomplete="off">
                                            <button type="button" class="btn btn-outline-primary btn-sm reason-chip" onclick="setDiemMoiReason(this, 'Đổi lệnh')">Đổi lệnh</button>
                                            <button type="button" class="btn btn-outline-primary btn-sm reason-chip" onclick="setDiemMoiReason(this, 'Lãnh vật tư')">Lãnh vật tư</button>
                                        </div>
                                    </div>
                    ...
                    <!-- Cấp dầu cho tàu vệ sinh và chạy pano -->
                    <div class="form-check form-switch mb-2">
                        <input class="form-check-input" type="checkbox" id="cap_them" name="cap_them" <?php echo ($formData['cap_them'] ? 'checked' : ''); ?>>
                        <label class="form-check-label" for="cap_them"><strong>Cấp thêm</strong></label>
                    </div>
                    <div id="cap_them_fields" class="border rounded p-3 mb-3" style="display:none;">
                        <div class="mb-3">
                        ...
                        <div class="mb-3" id="dia_diem_cap_them_wrapper">
                            ...
                            <input type="text" class="form-control" id="dia_diem_cap_them" name="dia_diem_cap_them"
                        ...
                        <div class="mb-3" id="ly_do_cap_them_wrapper" style="display:none;">
                            ...
                            <input type="text" class="form-control" id="ly_do_cap_them_khac" name="ly_do_cap_them_khac" autocomplete="off" placeholder="Nhập lý do...">
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            <label for="ly_do_cap_them_display" class="form-label">Lý do tiêu hao (tự động tạo)</label>
                            ...
                            <div class="form-text">
                                <i class="fas fa-info-circle me-1"></i>
                                Lý do sẽ được tự động tạo dựa trên loại và địa điểm bạn chọn
                            </div>
                        </div>
                    ...
                    document.querySelectorAll('input[name="loai_cap_them"]').forEach(function(radio) {
                        radio.addEventListener('change', function() {
                            ...
                            if (this.value === 'khac') {
                                diaDiemWrapper.style.display = 'none';
                                ...
                                if (lyDoKhacInput) lyDoKhacInput.setAttribute('required', 'required');
                                ...
                                if (diaDiemInput) diaDiemInput.setAttribute('required', 'required');
                    ...
                    function updateCapThemPreview() {
                        ...
                        const diaDiem = document.getElementById('dia_diem_cap_them')?.value.trim() || '';
                        const lyDoKhac = document.getElementById('ly_do_cap_them_khac')?.value.trim() || '';
                        const soLuong = document.getElementById('so_luong_cap_them')?.value || '';
                        const lyDoDisplay = document.getElementById('ly_do_cap_them_display');
                        ...
                        if (loai === 'bom_nuoc') {
                            ...
                            if (lyDoKhac) {
                                resultText = soLuong ? `CẤP THÊM: ${lyDoKhac} x ${soLuong} lít` : `CẤP THÊM: ${lyDoKhac}`;
                    ...
                    if (soLuongInput) {
                        soLuongInput.addEventListener('input', updateCapThemPreview);
                    }
                    ...
                    document.addEventListener('DOMContentLoaded', function() {
                        // Thiết lập required ban đầu (Ma nơ được chọn mặc định)
                        const diaDiemInput = document.getElementById('dia_diem_cap_them');
                        const lyDoKhacInput = document.getElementById('ly_do_cap_them_khac');
