            ...
            combined.forEach(row=>{
                if (row.type==='doan'){
                    ...
                    rows += `<tr>`+
                        ...
                        `<td><button type="button" class="btn btn-sm btn-outline-primary" onclick="openEditSegmentModal(${idx})" title="Sửa đoạn này"><i class="fas fa-edit"></i> Sửa</button></td>`+
                    `</tr>`;
                } else {
                    const c=row.data;
                    const idx = Number(c.___idx||0);
                    rows += `<tr class="table-warning">`+
                        ...
                        `<td><span class="text-muted">—</span></td>`+
                        `<td><button type="button" class="btn btn-sm btn-outline-primary" onclick="openEditSegmentModal(${idx})" title="Sửa cấp thêm này"><i class="fas fa-edit"></i> Sửa</button></td>`+
                    `</tr>`;
                }
            });
    ...
    (function initMonthPicker(){
      ...
      applyBtn && applyBtn.addEventListener('click', ()=>{
        ...
        const updateLyDoCapThemState = () => {
            ...
            setFieldState(lyDoCapThemInput, shouldShow, !shouldShow);
        };
        ...
        if (capThemCheckbox && capThemFields) {
            const toggleFields = () => {
                ...
                setFieldState(diaDiemCapThemInput, capOn, !capOn);
                setFieldState(soLuongCapThemInput, capOn, !capOn);
                updateLyDoCapThemState();
                ...
                // Vô hiệu hóa số chuyến khi không có hàng (khối lượng = 0)
                const khoiLuongInput = document.getElementById('khoi_luong');
                const soChuyenSelect = document.getElementById('so_chuyen');
                const capThemCheckboxForTrip = document.getElementById('cap_them');
                if (khoiLuongInput && soChuyenSelect) {
                    const updateSoChuyenState = () => {
                        const khoiLuong = parseFloat(khoiLuongInput.value) || 0;
                        const isCapThem = capThemCheckboxForTrip && capThemCheckboxForTrip.checked;
                        if (khoiLuong === 0 && !isCapThem) {
                            // Không có hàng và không phải Cấp thêm: vô hiệu hóa số chuyến
                            soChuyenSelect.disabled = true;
                            if (soChuyenSelect.value) {
                                soChuyenSelect.value = '';
                            }
                        } else {
                            // Có hàng hoặc đang ở chế độ Cấp thêm: cho phép nhập số chuyến
                            soChuyenSelect.disabled = false;
                        }
                    };
                    khoiLuongInput.addEventListener('input', updateSoChuyenState);
                    khoiLuongInput.addEventListener('change', updateSoChuyenState);
                    if (capThemCheckboxForTrip) {
                        capThemCheckboxForTrip.addEventListener('change', updateSoChuyenState);
                    }
                    updateSoChuyenState(); // Gọi ngay để set trạng thái ban đầu
                }
            ...
            loaiCapThemRadios.forEach(radio => {
                radio.addEventListener('change', updateLyDoCapThemState);
            });
            toggleFields();
        }
        updateLyDoCapThemState();
            ...
            */
            ...
            // Xử lý hiển thị trường khoảng cách thủ công khi cần thiết
            function checkAndShowManualDistance() {
                ...
                if (diemBatDau && diemKetThuc && khoangCachThuCongFields) {
                    // Kiểm tra xem có tuyến đường trực tiếp không
                    // Gọi API không filter theo keyword để lấy toàn bộ điểm nối từ A, sau đó kiểm tra exact-match ở client.
                    const url = `api/search_diem.php?keyword=&diem_dau=${encodeURIComponent(diemBatDau)}`;
                    
                    fetch(url)
                        .then(response => response.json())
                        .then(data => {
                            // Chỉ xem như có tuyến trực tiếp nếu trùng khớp CHÍNH XÁC điểm cuối (chuẩn hóa NFC + case-insensitive)
                            // Chuẩn hóa so sánh: NFC + bỏ dấu (NFD) + lower + trim để khớp tên có/không dấu
                            const norm = function(s){
                                const str = String(s||'').normalize('NFC');
                                // Bỏ dấu bằng NFD và loại bỏ combining marks Unicode (dải U+0300..U+036F để tương thích trình duyệt)
                                const noAccent = str.normalize('NFD').replace(/[\u0300-\u036f]/g, '');
                                // Loại bỏ ký tự không phải chữ/số và chuẩn hóa khoảng trắng để tăng khả năng khớp
                                const cleaned = noAccent.replace(/[^a-z0-9\s]/gi, ' ').replace(/\s+/g, ' ');
                                return cleaned.toLowerCase().trim();
                            };
                            const hasExact = data && data.success && Array.isArray(data.data)
                                ? data.data.some(function (it) { return norm(it.diem) === norm(diemKetThuc); })
                                : false;
                            const applyDisplay = function(found){
                                if (found) {
                                    // Có tuyến đường trực tiếp, ẩn trường khoảng cách thủ công nhưng KHÔNG xóa giá trị người dùng đã nhập
                                    khoangCachThuCongFields.style.display = 'none';
                                    if (khoangCachThuCongInput) { khoangCachThuCongInput.required = false; }
                                } else {
                                    // Không có tuyến đường trực tiếp, hiển thị trường khoảng cách thủ công
                                    khoangCachThuCongFields.style.display = 'block';
                                    if (khoangCachThuCongInput) { khoangCachThuCongInput.required = true; }
                                }
                            };
                            if (hasExact) { applyDisplay(true); return; }
                            ...
                            fetch(url2)
                                ...
                                .then(d2 => {
                                    ...
                                    const found2 = list2.some(function (it) { return norm(it.diem) === norm(diemKetThuc); });
                                    applyDisplay(found2);
                                })
                                .catch(() => { applyDisplay(false); });
                        })
                        .catch(error => {
                            console.error('Lỗi kiểm tra tuyến đường:', error);
                            // Nếu có lỗi, hiển thị trường khoảng cách thủ công để an toàn
                            khoangCachThuCongFields.style.display = 'block';
                    ...
                    // Chưa chọn đủ điểm, ẩn trường khoảng cách thủ công
