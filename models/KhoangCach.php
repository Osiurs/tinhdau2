...
class KhoangCach {
    ...
    private function getPointNameVariants($diem) {
        ...
        // 2. Tên đã loại bỏ phần trong ngoặc thường ()
        $diemNoParen = preg_replace('/\s*\([^)]*\)\s*$/', '', $diem);
        if ($diemNoParen !== $diem) {
            $variants[] = $this->normalizeString($diemNoParen);
        }
        
        // 3. Tên đã loại bỏ phần trong ngoặc fullwidth （）
        $diemNoFullParen = preg_replace('/\s*（[^）]*）\s*$/', '', $diem);
        if ($diemNoFullParen !== $diem && $diemNoFullParen !== $diemNoParen) {
            $variants[] = $this->normalizeString($diemNoFullParen);
        }
        
        // 4. Loại bỏ cả hai loại ngoặc
        $diemNoAllParen = preg_replace('/\s*（[^）]*）\s*$/', '', $diemNoParen);
        if ($diemNoAllParen !== $diemNoParen) {
            $variants[] = $this->normalizeString($diemNoAllParen);
        }
